-- 대기열 진입. 만료 정리 · 기존 토큰 재개 · 신규 순번 발급을 한 번에 끝낸다.
-- 정리할 토큰은 실행 시점에야 알 수 있어 token/member key 를 KEYS 로 못 받는다.
-- 대신 모든 key 가 같은 {scheduleId} hash tag 를 써서 한 slot 에 머문다.
...
-- tostring 은 큰 수를 지수 표기로 바꾼다. epoch millis 는 %d 로 찍는다.
...
-- 만료는 hardExpiresAt < now 다. 정확히 만료 시각인 토큰은 남긴다.
local stale = redis.call('ZRANGEBYSCORE', expiryKey, '-inf', '(' .. int(nowMillis))
for index = 1, #stale do
    forget(stale[index])
end

-- 대기열 인덱스는 살아 있는 토큰 중 가장 늦게 만료되는 것보다 오래 남아야 한다.
...
if existing then
    ...
    elseif fields[1] ~= 'WAITING' then
        -- 입장 허용 토큰은 이 PR 이 만들지 않는다. 들어오면 손대지 않고 그대로 돌려준다.
